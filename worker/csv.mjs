...
// Turn CSV text into a list of jobs. Narration is locked to Ava, so any legacy
// voice column is deliberately ignored.
export function jobsFromCSV(text) {
  const rows = parseCSV(text);
  if (!rows.length) return [];
  let start = 0, ti = 0, si = 1, sti = 2, hi = -1, mi = -1;
  ...
  if (first.indexOf("script") >= 0) {
    ...
    hi = first.indexOf("hook"); mi = first.indexOf("music");
    if (ti < 0) ti = 0;
    start = 1;
  }
  const out = [];
...
// lots of scenes rather than a few very long ones. The supplied reference changes
// scenes at roughly one short spoken sentence, so production uses about 12 words.
